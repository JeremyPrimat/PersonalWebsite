          ...
          <div className="flex flex-wrap justify-center gap-8">
            {displayPosts.map((post, index) => (
                <AnimatedSection key={post.id} delay={index * 0.2}>
                  <motion.article
                    className="bg-gray-50 dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-xl dark:shadow-gray-900/20 transition-all group w-full max-w-sm h-full flex flex-col"
                    whileHover={{ y: -8 }}
                    transition={{ type: 'spring', stiffness: 300 }}
                  >
